// src/app/(main)/kuliner/page.tsx (updated with recommendations)
...
function FilterContent({
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <label className="mb-2 block text-sm font-medium text-slate-700">Jenis Tempat</label>
        <div className="flex flex-wrap gap-2">
          <button
          ...
          {jenisOptions.map((j) => (
            <button
              key={j}
              onClick={() => onFilterChange('jenis', j)}
              className={cn(
                'rounded-full px-4 py-2 text-sm font-medium transition-all duration-200',
                filters.jenis === j
                  ? 'bg-brand-navy text-white shadow-sm'
      ...
      <div>
        <label className="mb-2 block text-sm font-medium text-slate-700">Cari Kuliner</label>
        <div className="relative">
          ...
          <input
            ...
            placeholder="Cari kuliner, nama tempat, atau menu..."
...
function KulinerPageContent() {
  ...
  const { data, isLoading, error, meta } = useKuliner({
  ...
  useEffect(() => {
    ...
    const newUrl = `/kuliner${params.toString() ? `?${params.toString()}` : ''}`
      ...
      <div className="container mx-auto px-4 pb-8 pt-32 md:px-6 md:pt-36 lg:px-8 lg:pt-32">
        <div className="mb-8 text-center lg:text-left">
          <h1 className="text-3xl font-bold text-brand-navy md:text-4xl">
            Kuliner Ciayumajakuning
          </h1>
          <p className="mt-2 text-slate-500 max-w-2xl lg:mx-0 mx-auto">
            Temukan berbagai tempat makan, cafe, oleh-oleh khas, dan kuliner terbaik
            dari setiap wilayah di Ciayumajakuning.
          </p>
        </div>
              ...
              <p className="text-sm text-slate-500">
                {!isLoading && !error && meta && (
                  <span>
                    ...
                    dari <span className="font-semibold">{meta.total}</span> kuliner
                  </span>
                )}
              </p>
              <button
            ...
            {error && !isLoading && (
              <div className="flex justify-center py-20">
                <EmptyState
                  ...
                  description="Terjadi kesalahan saat memuat data kuliner. Silakan coba lagi."
